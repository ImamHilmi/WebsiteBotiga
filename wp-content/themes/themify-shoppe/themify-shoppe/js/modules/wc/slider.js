...
(function($,document,Themify){
    ...
    const init=function(el) {
					const product =el.closest('.products'), 
						...
						big.className='swiper-container tf_carousel swiper-container-big tf_rel tf_overflow';
						thumbs.className='swiper-container tf_carousel swiper-container-thumbs tf_w';
						big_sw.className='swiper-wrapper tf_rel tf_w tf_h';
						thumb_sw.className='swiper-wrapper tf_rel tf_w tf_h tf_box';
								...
								if(len>0){
									for (let i=0;i<len;++i) {
										let slide_big=document.createElement('div'),
											...
											slide_big.className='swiper-slide tf_box';
											slide_thumb.className='swiper-slide tf_box';
									...
									if(len>1){
										...
										el.classList.add('slider-finish');
										big.classList.add('swiper-container-initialized');	
										spinner.remove();
									}
								}
								if(len<=1){
