...
class Themify_WC{
	...
	public static function share_items(){
		...
		<div class="product-share-wrap tf_inline_b tf_vmiddle">
				...
				<?php if($showQuick===true):?>
					 <a onclick="return false;" class="quick-look themify-lightbox" href="<?php echo get_permalink() ?>" rel="nofollow"><?php echo themify_get_icon('zoom-in','ti')?><span class="tooltip"><?php _e('Quick Look', 'themify'); ?></span></a>
				<?php endif;?>
				<?php if ($showSocial===true): ?>
					<?php 
					static $socialShare=null;
					if($socialShare===null){
						ob_start();
	...
	public static function loop_product_title(){
		...
		if($themify->hide_title!=='yes'){
			themify_post_title(array('tag'=>'h3','class'=>esc_attr( apply_filters( 'woocommerce_product_loop_title_classes', 'woocommerce-loop-product__title' ) )));
		}
	}
	...
	*/
	public static function add_to_cart_message( $message ){
		if ( isset( $_REQUEST['action'] ) && 'theme_add_to_cart' === $_REQUEST['action'] ) {
			...
			add_action( 'wp_ajax_theme_add_to_cart',array(__CLASS__,'ajax_add_to_cart_refresh') );
			add_action( 'wp_ajax_nopriv_theme_add_to_cart', array(__CLASS__,'ajax_add_to_cart_refresh') );
			$message = '';
		}
		return $message;
	...
	public static function pagination() {
		if ( wc_get_loop_prop( 'is_shortcode' ) ) {
            if('products'===wc_get_loop_prop( 'name' )){
			...
			if(!empty($product)){
				...
				foreach ( $attachment_ids as $attachment_id ) {
					...
					if ( $is_disabled ) {
						...
						$result['big'][] = themify_get_image('urlonly=1&w='.$width.'&h='.$height.'&src='.$src);
						$result['thumbs'][]=themify_get_image('urlonly=1&w=28&h=28&src='.$src);
					}
				}
