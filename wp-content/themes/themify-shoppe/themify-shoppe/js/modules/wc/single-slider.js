...
(function($,document,Themify){
	...
	init=function(item) {
		const items=item.getElementsByClassName('swiper-container'),
			main=items[0],
			thumbs=items[1];
		setTimeout(function(){
			zoomInit(main);
		},800);
		if(main.getElementsByClassName('swiper-slide').length<=1){
			main.classList.add('swiper-container-initialized');
			main.classList.remove('tf_hidden');
			Themify.lazyScroll(main.querySelectorAll('[data-lazy]'),true);
			if(thumbs){
				thumbs.classList.add('swiper-container-initialized');
				thumbs.classList.remove('swiper-container-initialized');
		...
		Themify.InitCarousel(instance.elements[0],{
			...
			thumbs:thumbs,
			onInit(){
							const form = isFormInit===null?this.el.closest('.product').getElementsByClassName('variations_form')[0]:null;
							if(form){
								const _this=this;
								let isInit=null;
