	...
	if( themify_is_woocommerce_active() ) {
		...
		$themify_script_vars+= apply_filters(
			...
			array(	'redirect'=> get_option( 'woocommerce_cart_redirect_after_add' ) === 'yes' ? wc_get_cart_url() : false,
				...
				'is_default_gallery'=>$gallerType === 'default',
		...
		$themify_script_vars['ajaxSingleCart']=is_product() && themify_check( 'setting-single_ajax_cart',true );
	}
...
function themify_theme_enqueue_footer(){
    ...
    if (  $menu_style!==false ) {
	if(in_array($menu_style, array('boxed','fadein-down','flip-down','flipin-left','flipin-right','slide-left-content','slidein-left','slidein-right','split','swing-left-to-right','swing-right-to-left','swing-top-to-bottom','swipe-left','swipe-right'),true)){
...
add_action( 'themify_comment_start', 'themify_comment_form_css',10,1 );
function themify_comment_form_css($post_id){
	remove_action( 'themify_comment_start', 'themify_comment_form_css',10,1 );
...
function themify_theme_get_sidebar_type(){
	...
	if ( is_page() ) {
		...
		if ( $is_wc_active===false && $page_sidebar ) {
...
function themify_add_tile_layout($options,$module){
	if($module->slug==='products' || $module->slug==='post'){
		...
		$bindig = array(
			'not_empty' => array(
				'show' => array('content_layout','masonry_post ','post_gutter')
			),
			'list-post' => array(
...
function themify_get_gallery_type() {
	return themify_get( 'setting-product_gallery_type','default',true );
}
